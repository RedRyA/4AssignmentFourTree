...
// sets z to x's left or right
     ...
     // checks to see if z and x have different balances and rotates accorddingly 
        if(!isSameSignBalance(x,z) &&z==x->pRight){
            leftRotate(t,z);
        } else if(z==x->pLeft)
        {
            rightRotate(t,z);
...
if(subTreeHeight(x)>subTreeHeight(y))
{
    return x;
}
else if(subTreeHeight(y)>subTreeHeight(x))
{
    return y;
}
else if (subTreeHeight(x)==subTreeHeight(y))
{
    return;
}else {
   exit(-1);  
...
bool isSameSignBalance(TNode* x, TNode* z){
    //optional TODO
    if(subTreeHeight(x)==subTreeHeight(z)){
        return true;
    } else{
        return false;
    }
